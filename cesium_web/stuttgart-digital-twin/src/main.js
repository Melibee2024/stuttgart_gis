...
// 2. Add placeholder OpenStreetMap 3D Buildings for Stuttgart
const buildings = await Cesium.Cesium3DTileset.fromIonAssetId(96188);

// Erase the specific generic placeholder building matching Bau 4's OSM footprint
buildings.style = new Cesium.Cesium3DTileStyle({
    show: {
        evaluate: function(feature) {
            // Hide ONLY the generic OSM placeholder element for Bau 4
            if (feature.getProperty('elementId') === 25155812) {
                return false;
            }
            return true; // Show all other buildings in Stuttgart normally
        }
    }
});
...
// =========================================================================
// LOAD SEAMLESSLY ALIGNED HFT BAU 4 BIM MODEL FROM CESIUM ION
// =========================================================================
try {
    // Streams your custom asset using your saved dashboard adjustments
    const bau4Tileset = await Cesium.Cesium3DTileset.fromIonAssetId(4810292);
    viewer.scene.primitives.add(bau4Tileset);
    console.log("🎯 Bau 4 BIM Model successfully streamed from Ion using dashboard adjustments.");
} catch (error) {
    console.error("Asset loading error for Bau 4:", error);
}
// =========================================================================

// 3. Fly the camera straight to the HFT Stuttgart Campus area
viewer.camera.flyTo({
    destination: Cesium.Cartesian3.fromDegrees(9.1727, 48.7801, 400),
    orientation: {
...
handler.setInputAction(async function (click) {
    ...
    if (Cesium.defined(pickedFeature)) {
        // Extract the target key identifier attribute assigned by the feature geometry
        ...
        if (buildingGmlId) {
            try {
                // Query your live Node.js database bridge running on port 5000
                ...
                // Inject the live PostGIS / ALKIS / QField rows directly into the UI panel
